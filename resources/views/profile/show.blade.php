...
<head>
    ...
    <title>{{ $user->name }}'s Profile - Artrium</title>
...
<body class="bg-black text-white min-h-screen pt-20"> 
    ...
    <nav class="fixed top-0 w-full bg-black bg-opacity-90 z-50 px-6 py-4 border-b border-gray-800">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div class="hidden md:flex space-x-8">
                ...
                <a href="{{ route('profile.show') }}" class="text-yellow-400 font-semibold transition">Profile</a> {{-- Link Profile Baru --}}
            </div>
    ...
    <header class="profile-header py-10">
        <div class="max-w-4xl mx-auto text-center">
            ...
            {{-- Statistik Sederhana --}}
            <p class="text-gray-500">{{ count($myImages) }} Karya telah diunggah</p>
            
            {{-- Tombol Edit Profil (Jika ada rutenya) --}}
            {{-- <a href="{{ route('profile.edit') }}" class="mt-4 inline-block px-4 py-2 bg-gray-700 text-white rounded-full hover:bg-gray-600 transition">Edit Profil</a> --}}
        </div>
    </header>
    ...
    <main class="py-12 px-6">
        <div class="max-w-7xl mx-auto">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    ...
                    @foreach ($myImages as $image)
                        <div class="relative group overflow-hidden rounded-lg bg-gray-900 border border-gray-800 hover:border-yellow-400 transition">
                            <img src="{{ $image['image_url'] ?? '#' }}" alt="{{ $image['title'] }}" class="w-full card-image transform group-hover:scale-[1.05] transition duration-500">
                            <div class="p-4">
                                ...
                                {{-- 🎯 TOMBOL KONTROL: EDIT & DELETE (Hanya di halaman ini) --}}
                                <div class="mt-3 flex justify-between items-center">
                                    <a href="{{ route('images.edit', $image['id']) }}" class="text-xs px-3 py-1 bg-yellow-400 text-black rounded hover:bg-yellow-500 font-semibold">
                                        Edit
                                    </a>
                                    ...
                                    <a href="{{ route('images.show', $image['id']) }}" class="text-xs px-3 py-1 bg-gray-700 text-white rounded hover:bg-gray-600 font-semibold">
                                        View
                                    </a>
                                </div>
                            </div>
            ...
            @else
                <p class="text-center text-gray-500 text-xl py-10">Anda belum mengunggah karya apapun. Yuk, unggah karya pertama Anda!</p>
            @endif
